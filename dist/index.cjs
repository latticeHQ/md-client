...
async function signEd25519(message, privateKey) {
  const sig = await ed__namespace.signAsync(message, privateKey);
  return btoa(String.fromCharCode(...sig));
}
function signECDSA(message, privateKey, curve) {
  const curves = { p256: nist_js.p256, p384: nist_js.p384, p521: nist_js.p521 };
...
async function verifySignature(parsedKey, message, signature) {
  try {
    switch (parsedKey.type) {
      case "ed25519":
        return await ed__namespace.verifyAsync(signature, message, parsedKey.keyBytes);
      case "ecdsa-p256":
        return nist_js.p256.verify(signature, message, parsedKey.keyBytes, { prehash: true });
      case "ecdsa-p384":
        return nist_js.p384.verify(signature, message, parsedKey.keyBytes, { prehash: true });
      case "ecdsa-p521":
        return nist_js.p521.verify(signature, message, parsedKey.keyBytes, { prehash: true });
      default:
        return false;
    }
  } catch {
...
exports.createSignatureEnvelope = createSignatureEnvelope;
...
exports.generateMutateKey = generateMutateKey;
